// ===== Utilities =====
    ...
    // ===== Audio (WebAudio simple beeps, no assets) =====
    const AudioKit = (()=>{
    ...
    const ui = {
      score: $('#score'), hi: $('#hi'), mode: $('#mode'), speed: $('#speed'), state: $('#state'),
      speedVal: $('#speedVal'), gridVal: $('#gridVal'), obsVal: $('#obsVal')
    };

    let grid = 24;            // cells per row/col
    let speed = 10;           // ticks per second
    let showGrid = true;
    let mode = 'classic';     // classic | wrap | obstacles
    let obstacleDensity = 8;  // percentage of cells
    let soundOn = true;
    let vibeOn = true;
    ...
    const state = {
      ...
      step:0,
      acc:0,
      cell(){ return Math.floor(canvas.width / grid) },
      snake:[],
      dir:{x:1,y:0},
      pending:null,
      food:null,
    ...
    // ===== Responsive Canvas =====
    const resize = () => {
      const size = Math.floor(document.querySelector('.board-wrap').clientWidth - 32);
      const dpr = Math.min(2, window.devicePixelRatio || 1);
      const px = Math.max(280, size);
      canvas.width = px * dpr; canvas.height = px * dpr; canvas.style.width = px + 'px'; canvas.style.height = px + 'px';
      ctx.setTransform(dpr,0,0,dpr,0,0);
    };
    window.addEventListener('resize', resize, { passive:true });
    resize();

    // ===== Storage Keys per config =====
    ...
    function genFood(){
      let p; do { p = randomCell(); } while(occupied(p) || isObstacle(p));
      state.food = p;
    }

    function genBonus(){
      // 20% chance after each food eat
      if(Math.random()<0.2){
        let p; do { p = randomCell(); } while(occupied(p) || isObstacle(p) || same(p, state.food));
        state.bonus = { ...p, t: Date.now(), life: 8000 }; // 8s lifetime
      }
    }

    function genObstacles(){
      state.obstacles.clear();
      if(mode!=='obstacles' || obstacleDensity<=0) return;
      const cells = grid*grid;
      const count = Math.floor(cells * (obstacleDensity/100));
      let i=0; while(i<count){
        const p = randomCell();
        const k = key(p);
        if(!state.obstacles.has(k) && !occupied(p)){
          state.obstacles.add(k); i++;
        }
      }
    }

    function occupied(p){
      return state.snake.some(s => s.x===p.x && s.y===p.y);
    }
    ...
    function reset(){
      state.score = 0; state.over=false; state.running=false; state.acc=0; state.step=0; state.bonus=null;
      initSnake(); genFood(); genObstacles();
      loadBest();
      updateBadges();
      render();
    }
    ...
    function tick(){
      ...
      const next = state.pending || state.dir; // avoid double turns per tick
      state.dir = next; state.pending = null;
      ...
      if(same(head, state.food)){
        state.score += 1; genFood(); genBonus();
        AudioKit.play(660, .06, 'square', .04);
        vibrate(8);
      } else if(state.bonus && same(head, state.bonus)){
        state.score += 5; state.bonus = null;
        AudioKit.play(880, .10, 'sawtooth', .05);
        vibrate(12);
      } else {
        state.snake.shift(); // normal move
      }

      // bonus expiry
      if(state.bonus && Date.now() - state.bonus.t > state.bonus.life){ state.bonus = null }

      // level up speed a bit every 10 points
    ...
    function gameOver(){
      state.over = true; state.running=false; AudioKit.play(140, .18, 'triangle', .05); vibrate(20);
      $('#btn-pause').textContent = 'Play ▷';
      $('#btn-pause-mobile').textContent = '⏯';
      updateBadges();
    }
    ...
    function render(){
      ...
      if(showGrid){
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        for(let i=1;i<grid;i++){ ctx.moveTo(i*c,0); ctx.lineTo(i*c, canvas.height); }
        for(let i=1;i<grid;i++){ ctx.moveTo(0,i*c); ctx.lineTo(canvas.width, i*c); }
        ctx.stroke();
      }

      // obstacles
      ctx.fillStyle = getCSS('--bad');
      state.obstacles.forEach(k=>{ const [x,y]=k.split(',').map(Number); ctx.fillRect(x*c,y*c,c,c) })

      // food
      if(state.food){ drawCell(state.food, getCSS('--good')) }
      if(state.bonus){ drawCell(state.bonus, '#f59e0b') }

      // snake with head accent
      const snakeColor = getCSS('--accent');
      state.snake.forEach((s,i)=>{
        const tone = i===state.snake.length-1 ? snakeColor : shade(snakeColor, -12);
        drawCell(s, tone);
      })
      ...
      if(!state.running){
        ...
        ctx.fillText(state.over ? 'Game Over — Press Restart or Play' : 'Press Play to Start', canvas.width/2, canvas.height/2);
      }
    }
    ...
    function turn(vec){
      // prevent 180°
      const cur = state.dir;
      if(cur.x + vec.x === 0 && cur.y + vec.y === 0) return; // opposite
      state.pending = vec;
    }
    ...
    window.addEventListener('keydown', (e)=>{
      ...
      else if(e.key==='o' || e.key==='O'){ setMode('obstacles') }
      else if(e.key==='w' || e.key==='W'){ setMode(mode==='wrap'?'classic':'wrap') }
    });
    ...
    canvas.addEventListener('touchmove', e=>{
      ...
      if(Math.hypot(dx,dy) > 24){
        if(Math.abs(dx)>Math.abs(dy)) turn(dx>0?{x:1,y:0}:{x:-1,y:0}); else turn(dy>0?{x:0,y:1}:{x:0,y:-1});
        touchStart=null;
      }
    }, {passive:true})
    ...
    function updateBadges(){
      ui.score.innerHTML = `<small>Score</small> ${state.score}`;
      ui.hi.innerHTML = `<small>Best</small> ${state.best}`;
      ui.mode.innerHTML = `<small>Mode</small> ${mode.charAt(0).toUpperCase()+mode.slice(1)}`;
      ui.speed.innerHTML = `<small>Speed</small> ${speed.toFixed(0)}`;
      ui.state.innerHTML = `<small>Status</small> ${state.over? 'Game Over': state.running? 'Playing':'Ready'}`;
    }

    function togglePause(){
      state.running = !state.running && !state.over;
      $('#btn-pause').textContent = state.running ? 'Pause ❚❚' : 'Play ▷';
      $('#btn-pause-mobile').textContent = '⏯';
      if(state.running) { AudioKit.resume(); AudioKit.play(520,.05,'square',.03) }
      updateBadges();
    }

    function setMode(m){
      mode = m; $('#sel-mode').value = mode; reset();
    }
    ...
    $('#btn-theme').addEventListener('click', ()=>{
      ...
      const themes = ['neon','retro','contrast',''];
